...
function getAlias(props) {
  if (props.updatingNickname) {
    return props.nicknameTemp;
  }
...
export function Form(props) {
    return (
      <form id="postform" action="javascript:void(0)" onSubmit={props.onSubmit}>
        <FormInput id="postform-textarea" form="postform" value={props.msg} onChange={props.handleChange} name="content" placeholder="Chat">{props.msg}</FormInput>
        <FormOpt className="below-textarea">
          <NicknameOpt className={"nickname-option accept " + (props.updatingNickname ? 'active' : '')} onClick={props.nicknameAccept}>
          ...
          <NicknameOpt className={"nickname-option reject " + (props.updatingNickname ? 'active' : '')} onClick={props.nicknameReject}>
          ...
          <PostingAs>Posting as: 
            <Nickname type="text" value={getAlias(props)} onChange={props.changeNickname} onClick={props.nicknameUpdate} />
          </PostingAs>
          <CharCount>{props.msg.length}/256</CharCount>
